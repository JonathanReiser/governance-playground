...
- The entangler was generated by the wrong operator, inverting the classical game
  at maximal entanglement. Corrected to `D = U(pi, 0)`; the corner test now passes
  to 4.4e-16 across 201 gammas, where the previous generator failed at 1.0.
- The corner test itself was specified at `gamma = 0` only — precisely where that
  error is invisible. It now sweeps a dense grid.
- The payoff was zero-sum, which made the stated advantage objective unreachable
  in principle. Replaced with a bimatrix: the seat-symmetrised board result plus a
  non-zero-sum cost of engagement.
- The policies, costs and payoff matrix were frozen (`frozen-policy-payoff-spec.md`).
- Release scope fixed to implementation-only.
- The hardware backend was pinned rather than selected by `least_busy`.
- The participant control scheme was fixed to a four-operation menu.
...
Those issues were raised and addressed; the corrections are recorded in "What
changed from 0.1" above and in the commit history rather than folded silently
into the text.

This file is now a frozen design. It is **not** a research result, not evidence
of quantum cognition, and not yet a licence to collect: see the three
outstanding items in the header. A preregistration binds a specific run to a
specific hypothesis before entropy is drawn — that step belongs with the
implementation, using the existing `scripts/prereg.js` machinery, and has not
been taken.
