...
**Data collection:** all six questions are now settled and the choices behind
them are frozen. What remains before collection is committing this as v1.0 and
confirming the pinned backend is still live — `ibm_marrakesh` was verified on
2026-08-23, and IBM retires devices. Nothing further is undecided.
...
5. ~~Continuous controls or a preregistered menu?~~ **Resolved — a
   preregistered menu of four operations**, frozen here:

   | operation | `(theta, phi)` | matrix |
   |---|---|---|
   | `C` | `(0, 0)` | identity |
   | `D` | `(pi, 0)` | `[[0,1],[-1,0]]` |
   | `M` | `(pi/2, 0)` | real rotation, an even classical mix |
   | `Q` | `(0, pi/2)` | `diag(i, -i)` |

   `C` and `D` are mandatory, not decorative: they are the two classical pure
   policies, so a participant can reach the classical corners and the
   classical-limit regression test describes a state the interface can actually
   produce. `M` is the classical mixed strategy. `Q` is the Eisert-Wilkens-
   Lewenstein operation, included so the protocol's own subject matter is
   reachable rather than hypothetical.

   Verified against the frozen bimatrix, with the entangler generated by `D`:

   - at `gamma = 0`, `Q` produces payoffs identical to `C` in every cell — phase
     is inert, as the classical limit requires — and the unique pure Nash within
     the menu is `(D, D)` at `(-2/3, -2/3)`, the dilemma;
   - at `gamma = pi/2`, the unique pure Nash becomes `(Q, Q)` at `(0, 0)`, which
     is `R`, and maps under the spec's `x -> 3x+3` to the canonical `(3, 3)`.

   **Interface labels must not be these letters.** The draft already forbids
   telling a participant that a setting is intelligent, optimal or "more
   quantum"; `C`/`D`/`M`/`Q`, "cooperate"/"defect", or anything naming `Q` as the
   quantum option would do exactly that. Present four neutral, fixed-order,
   non-suggestive labels, and record the mapping in the run record rather than in
   the interface.

   **What this menu does not establish.** `(Q, Q)` being a Nash equilibrium
   *within four options* is far weaker than being one in the strategy space. It
   is weaker even than the two-parameter family, which is itself the restriction
   Benjamin & Hayden's objection targets — under full SU(2) `(Q, Q)` is not an
   equilibrium at all. The menu makes the EWL structure reachable; it does not
   make it true, and release 1 claims nothing about it either way.
