 ...
 * deploy actually reads real starting values from.
 *
 * Duplicated in server/scenarioOverrides.js rather than shared — same
 * module-system split (this is ESM, the server is CommonJS) and same
 * reasoning demoDeploy.js's own header comment already gives for its
 * duplicated deploy logic. Keep the two in sync by hand if this ever
 * changes.
 */
...
export function applyStartingConditionOverride(scenario, overrideId) {
